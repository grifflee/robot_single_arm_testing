...
ROS and no sudo:
  ...
  3. run xacro (which ships with genesis-world) for dof=4 and dof=5
...
UPSTREAM_URDF = "Lynxmotion/LSS-ROS2-Arms lss_arm_description/urdf/lss_arm.urdf.xacro"
DOFS = (4, 5)
...
def main(cfg: Config) -> None:
    ...
    try:
        ...
        for dof in DOFS:
            ...
            # xacro stamps the absolute source path into a banner comment; the
            # clone lives in a fresh mktemp dir every run, so leaving it in means
            # a regenerated URDF never diffs clean against the committed one.
            # Point the banner at the upstream URL instead.
            urdf = re.sub(r"(autogenerated by xacro from ).*?( \|)",
                          rf"\1{UPSTREAM_URDF}\2", urdf)
        ...
        print(f"wrote {_rel(cfg.out_dir / 'meshes')}")
        _verify(cfg.out_dir)
    finally:
        if cfg.keep_temp:
            print(f"kept {tmp}")
...
def _xacro_cmd() -> list[str]:
    """genesis-world already depends on xacro, so the venv almost always ships
    the ``xacro`` console script next to the interpreter -- use that and skip the
    network. Note the package has no ``__main__``, so ``-m xacro`` does NOT work;
    it has to be the console script. Fall back to a throwaway `uv tool run` copy
    if the venv somehow lacks it."""
    import sys

    console = Path(sys.executable).parent / "xacro"
    if console.is_file():
        return [str(console)]
    print("no xacro console script in this venv; falling back to `uv tool run --from xacro`")
    return ["uv", "tool", "run", "--from", "xacro", "xacro"]
